...
* Requirements
   1) Get the Total Number of Hits, Visits and Usage of all the
      ...
      *Note* : Observe =GET /labs/cse01/= column to know the visit count.
      ...
      - Usage :: 1) A Visit to the experiment of a lab and a Visit to
                 the simulation of the same lab. Considering both
                 visits are from same <ip-address>, and in 'x' amount
                 of time.
		 2) If a user visits an experiment page and later the simulation
                 page on the same day, then it is being counted as a
                 usage.  
                 3) The sequence of visits to the simulation page
                    before or after the experiment page is not
                    considered.
   ...
   2) Similarly, get the Total Number of Hits, Visits and Usage of
      each virtual-lab.
...
* TODO Display Analytics on Vlabs landing page     
...
#+END_SRC
#+BEGIN_SRC html: web-page/index.html 
<html>
  <head>
