...
* Requirements
  1) Get the Total Number of Hits, Visits and Usage of all the
     ...
     - HIT :: Each line in a log file of a particular lab is a single
             ...
             The following example shows the sample log lines of
              =cse01= lab with 4 hits.
              #+BEGIN_EXAMPLE
     ...
     - Visit :: A Hit to a lab web page either html or php pages
		...
		#+END_EXAMPLE
                ...
                *Note* : Observe =GET /labs/cse01/= column to know the
                visit count.

     - Usage :: 
                1) A Visit to the experiment of a lab and a Visit to
                   the simulation of the same lab. Considering both
                   visits are from same <ip-address>, and in 'x'
                   amount of time.
                2) If a user visits an experiment page and later the
                   simulation page on the same day, then it is being
                   counted as a usage.
  ...
  *Note* : Hits, Visits and Usage is calculated from the logs of the
  labs which are hosted on IIIT-H Infrastrure and AWS. Labs on IIIT-H
  are not at Level-5, hence these labs are running on Individual
  containers.  The labs which are at level-5 are hosted on AWS.
