...
** Usage
   ...
   + If a user visits an experiment page and the simulation page with
     minimum of 'x' time and maximum of 1 day, then it is counted as a
     usage.
   ...
   + The lab existing with multiple simulations per experiment, In
     such cases visit to any one simulation (among many) will be
     considered as usage.
   + The labs, which are deployed with only simulations, then a visit
     to the simulation is itself considered as usage.
   + The labs, with-out simulations are not considered as
     usage. (Although hits and visits will be calculated)
   + The number of users accessing a lab from the same <ip-address>
   ...
   + The labs with tasks instead of simulations, then each visit to a
     task is counted as usage.
