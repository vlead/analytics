...
* Introduction
  ...
  *Note*: At the time of writing this document, vlab web page is
  [[http://www.vlabs.ac.in][www.vlabs.ac.in]]
...
** Usage
   ...
   follows:
           ...
           simulation page with minimum of 'x' time
           ...
           vice versa).
	4) The lab existing with multiple simulations per experiment,
           In such cases visit to any one simulation (among many) will
           be considered as usage.
	5) The labs, which are deployed with only simulations, then a
           visit to the simulation is itself considered as usage.
	6) The labs, with-out simulations are not considered as
           usage. (Although hits and visits will be calculated)
	7) The number of users accessing a lab from the same
           <ip-address> (either from the same Institute or College),
           then the minimum number of the experiment page visits and
           simulation page visits is considered as usage.
	8) The labs with tasks instead of simulations, then each visit
   ...
   *Note* : Hits, Visits and Usage is calculated from the logs of the
            labs which are hosted on IIIT-H Infrastrure and AWS. Labs
            on IIIT-H are not at Level-5, hence these labs are running
            on Individual containers.  The labs which are at level-5
            are hosted on AWS.
...
** Total usage of all virtual-labs
   Get the Total Number of Hits, Visits and Usage of all the
   virtual-labs.
...
** Displaying of Analytics on Vlabs Web page
   The following are the requirements for displaying of Analytics on vlabs web page:
   1) The vlabs landing page should display numbers for: Labs, Experiments, Visits, and Usage.
      *Note*: only "Visits" and "Usage" will be provided by analytics service model.      
   2) Upon clicking on Usage, it should open a web page with Analytics of
      all labs.  
...
** COMMENT OLD Hits, Visits and Usage based on apache combined logs
...
*** Restructuring of logs
    ...
    week.  Thus for each lab there could be several log files contains
    logs worth one week of lab usage.  Further since initially on base
    machines several labs were hosted together, it is possible to have
    logs of many labs in one single log file.

    In case a single file has logs of many labs, then logs for each
    lab are separated using lab-ID (Eg cse01).  If a single log file
    contains only logs of particular lab then all logs from that file
    are treated appropriately.

    Before any analysis is done first bot and non ok response lines
    are removed.  For removing bots 'User-agent field' is searched for
    strings such as nagios or bot.  Then for non-ok response code any
    response that does not starts with 2 is treated as non-hit.  

    After all bot and non-hit lines are removed, the remaining logs
    are split as per each lab using lab-ID.  Thus even if a lab's logs
    are spread across various files, they are all combined in one
    single larger log file with name lab-ID. 
