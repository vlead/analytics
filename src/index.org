...
** Apache combined log format
   Apache combined log format is as follows:
   TODO
...
** Usage
   ...
   follows:
   TODO
** Month-wise experiment-wise usage
   TODO
** Total usage of lab across all experiments for a given month
   TODO 
** Total usage of lab across all months
   TODO
** Total usage of all virtual-labs
   TODO 
** Adding a new Lab for analytics
   TODO
** Stopping analytics for a lab
   TODO
** Changing analytics program code / experiment information, etc.
   TODO

** OLD Hits, Visits and Usage based on apache combined logs
...
**** (STEP:all_list) Prepare list of all log file names present on base2 http container log folder :Done:
     First we look at folder where log files are copied from http
     container of base2 by rsync cron job on analytics node and build a
     list of all log-files present.

**** (STEP:positive_filter) Filter irrelevant log files		       :Done:
    Since base2 logs have both actual logs files created by apache and
...
The output of program after this step is:
#+BEGIN_EXAMPLE
...
The output of program after this steps is:
#+BEGIN_EXAMPLE
...
**** (STEP:read_line_count) Read old line count stored from previous analysis :Done:
     ...
     *Note that in the document =read= is used to refer to numbers / values read from previous files.  At the same time =get= is used to refer to numbers/values read from current process data.*
...
**** (STEP:get_line_count) Get number of lines for each log file       :Done:
     ...
     is relevant and usable (=total_lines=)
...
**** (STEP:month_wise) Split all log lines such that they are stored in respective month folders :Done:
     ...
     This function assumes logs are in combined format and hence Dates
     are in =[18/Aug/2013:09:06:47","+0530]= format.  If this
     assumption is not met then the program will not work.
...
**** (STEP:get_hit_count) Get hit count (month-wise) from log files for each lab :Done:
     ...
     and prepare a big list of lab-wise month-wise hits.
...
**** (STEP:get_visit_count) Get visit count (month-wise) from log files for each lab :Done:
     ...
     visits.  Hence counting number of lines in remaining file gives
     number of visits for each lab (month-wise).
...
-define(Aws_file_lab_id_regexp_list, [    
					  ...
					  {"em-coep.virtual-labs.ac.in-access_log", [{".", "em"}]},
					  {"em-iitr.virtual-labs.ac.in-access_log", [{".", "em"}]},
					  ...
					  {"mm-coep.virtual-labs.ac.in-access_log", [{".", "mm"}]},
					  {"mm-nitk.virtual-labs.ac.in-access_log", [{".", "mm"}]},
					  ...
					  {"sl-coep.virtual-labs.ac.in-access_log", [{".", "sl"}]},
					  {"sl-iitr.virtual-labs.ac.in-access_log", [{".", "sl"}]},
					  ...
					  {"va-coep.virtual-labs.ac.in-access_log", [{".", "va"}]},
					  {"va-iitk.virtual-labs.ac.in-access_log", [{".", "va"}]},
					  ...
					  {"vp-dei.virtual-labs.ac.in-access_log", [{".", "vpvp"}]},
...
-define(Lab_id_exp_info_list, [
				...
				[
				 ...
				 {"Expression Evaluation", "/exp2/index.php", "/exp2/index.php.*section=Simulation.*"},
				 {"Sorting Algorithm", "/exp3/index.php", "/exp3/index.php.*section=Simulation.*"},
				 {"Polynomial Operations", "/exp4/index.php", "/exp4/index.php.*section=Simulation.*"},
				 {"Search Trees", "/exp5/index.php", "/exp5/index.php.*section=Simulation.*"},
				 {"Expression Trees", "/exp6/index.php", "/exp6/index.php.*section=Simulation.*"},
				 {"Graph Traversals", "/exp7/index.php", "/exp7/index.php.*section=Simulation.*"},
				 {"Shortest Paths in Graphs", "/exp8/index.php", "/exp8/index.php.*section=Simulation.*"},
				 {"Spanning Trees in Graphs", "/exp9/index.php", "/exp9/index.php.*section=Simulation.*"}
				]
			       },
			       {"cse02",
				[
				 {"Numerical Integration ( Area Under Curve )", "/exp1/index.html", "/exp1/simulation/index.html"},
				 {"Functions", "/exp2/index.html", "/exp2/simulation/index.html"},
				 {"Loops (Factorial Calculation)", "/exp3/index.html", "/exp3/simulation/index.html"},
				 {"Arrays", "/exp4/index.html", "/exp4/simulation/index.html"},
				 {"Structure", "/exp5/index.html", "/exp5/simulation/index.html"},
				 {"Basic Control Flow", "/exp6/index.html", "/exp6/simulation/index.html"},
				 {"Strings", "/exp7/index.html", "/exp7/simulation/index.html"},
				 {"Pointers", "/exp8/index.html", "/exp8/simulation/index.html"},
				 {"Recursion", "/exp9/index.html", "/exp9/simulation/index.html"},
				 {"Expression Evaluation", "/exp10/index.html", "/exp10/simulation/index.html"}
				]
			       },
			       {"cse15",
				[
				 {"Adder Circuit", "/exp1/index.php", "/exp1/exp1.html"},
				 {"Multiplexer", "/exp2/index.php", "/exp2/index.php.*section=Experiment.*"},
				 {"Decoder with 7 segment display", "/exp3/index.php", "/exp3/index.php.*section=Experiment.*"},
				 {"ALU with function", "/exp4/index.php", "/exp4/index.php.*section=Experiment.*"},
				 {"Comparator", "/exp5/index.php", "/exp5/index.php.*section=Experiment.*"},
				 {"JK Flip-Flop", "/exp6/index.php", "/exp6/index.php.*section=Experiment.*"},
				 {"Registers", "/exp7/index.php", "/exp7/index.php.*section=Experiment.*"},
				 {"Counters", "/exp8/index.php", "/exp8/index.php.*section=Experiment.*"},
				 {"Multipliers", "/exp9/index.php", "/exp9/index.php.*section=Experiment.*"},
				 {"State Diagrams", "/exp10/index.php", "/exp10/index.php.*section=Experiment.*"}
				]
			       },
			       %%TODO "cse19"
			       {"cse18",
				...
				[
				 ...
				 {"Data Clustering: K-means, MST based", "/exp10/index.php", "/exp10/index.php.*section=Experiment.*"}
			       ...
			       {"emt",
				[
				 {"Coulomb's Law", "/Experiment.php.*code=C001", "/Experiment.php?tid.*code=C001"},
				 {"Electrostatic Fields I", "/Experiment.php.*code=C002", "/Experiment.php?tid.*code=C002"},
				 {"Electrostatic Fields II", "/Experiment.php.*code=C003", "/Experiment.php?tid.*code=C003"},
				 {"Boundary Conditions", "/Experiment.php.*code=C004", "/Experiment.php?tid.*code=C004"},
				 {"Electric Field in Material Space", "/Experiment.php.*code=C005", "/Experiment.php?tid.*code=C005"},
				 {"Magnetostatic Fields", "/Experiment.php.*code=C006", "/Experiment.php?tid.*code=C006"},
				 {"Application of Faraday's Laws", "/Experiment.php.*code=C007", "/Experiment.php?tid.*code=C007"},
				 {"Wave Propagation", "/Experiment.php.*code=C008", "/Experiment.php?tid.*code=C008"},
				 {"Distance and Altitude Measurement", "/Experiment.php.*code=C009", "/Experiment.php?tid.*code=C009"},
				 {"Plane Waves - Reflection and Refraction", "/Experiment.php.*code=C010", "/Experiment.php?tid.*code=C010"}
				]
			       },
			       {"cse29",
				[
...
get_experiment_usage_for_lab_for_month_helper(Process_folder, Lab1, File1, Experiment1) ->
    ...
    {Exp_visit, Sim_visit, Usage}=get_usage_from_file(Input1, MP1, MP2, []),
    file:close(Input1),
...
%%% month.
get_usage_from_file(Input1, MP1, MP2, Result) ->    
    case file:read_line(Input1) of
	eof ->
	...
	{ok, Data} ->
		...
		{match, _} ->
		    ...
		    get_usage_from_file(Input1, MP1, MP2, Result2);
		nomatch ->
...
main() ->
    ...
    Command1=lists:flatten(io_lib:format("rm -rf ~s; mkdir -p ~s", [?Output_folder, ?Output_folder])),
    os:cmd(Command1),
...
** Removing a lab
   TODO
** Re-running entire analytics program again for all logs
   TODO
